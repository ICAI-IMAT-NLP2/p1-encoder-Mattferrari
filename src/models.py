...
class AttentionHead(nn.Module):
    ...
    def scaled_dot_product_attention(self, q, k: torch.Tensor, v):
        ...
        scores = q@k.transpose(1,2)/torch.sqrt(torch.tensor(dim_k))
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model: int, num_attention_heads: int):
        ...
        self.heads = nn.ModuleList([AttentionHead(d_model=d_model, 
                                                  d_k=d_qkv, 
                                                  d_q=d_qkv, 
                                                  d_v=d_qkv) 
    ...
    def forward(self, hidden_state):
        ...
        """
        x_ = torch.cat([head(hidden_state) for head in self.heads], dim=-1)
        x = hidden_state + self.output_linear(x_)
        return x
...
class FeedForward(nn.Module):
    ...
    def __init__(self, d_model: int, intermediate_size: int):
        ...
        self.linear_2 = nn.Linear(in_features=intermediate_size,
                                  out_features=d_model)
        self.gelu = nn.GELU()
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        hidden_state = self.attention(self.layer_norm_1(x)) + self.layer_norm_1(x)

        
        # Apply layer normalization and then apply feed-forward network
        x = self.feed_forward(self.layer_norm_2(hidden_state)) + self.layer_norm_2(hidden_state)
...
class Embeddings(nn.Module):
    ...
    def __init__(self, vocab_size: int, max_position_embeddings: int, d_model: int):
        ...
        self.position_embeddings = nn.Embedding(num_embeddings=max_position_embeddings, embedding_dim=d_model)
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        position_ids = torch.tensor([i for i in range(seq_length)])
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, vocab_size: int, max_position_embeddings: int, d_model: int,
        ...
        self.embeddings = Embeddings(vocab_size=vocab_size, 
                                     ...
                                     d_model=d_model)
        lista = [TransformerEncoderLayer(d_model=d_model,
                                        num_attention_heads=num_attention_heads,
                                        intermediate_size=intermediate_size) for _ in range(num_hidden_layers)]
        self.layers = nn.ModuleList([self.embeddings, *lista])
...
class TransformerForSequenceClassification(nn.Module):
    ...
    def __init__(self, vocab_size: int, max_position_embeddings: int, d_model: int,
        ...
        self.transformer_encoder = TransformerEncoder(vocab_size=vocab_size,
                                                      max_position_embeddings=max_position_embeddings,
                                                      d_model=d_model, 
                                                      num_attention_heads=num_attention_heads,
                                                      intermediate_size=intermediate_size,
                                                      num_hidden_layers=num_hidden_layers)
        self.classifier = ClassificationHead(d_model=d_model, num_classes=num_classes, dropout_prob=dropout_prob)
